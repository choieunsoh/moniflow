...
| **2. Client PWA**      | browser OPFS (WASM)  | static host _or_ installed PWA | install to home screen          | large (Plan 1+2)            | $0 (CF Pages / GH Pages / Vercel hobby)      | ~1 MB WASM load + worker boot on first visit, then every read/write is local — instant and fully offline. Best steady-state. | True offline + installed feel, single device     |
...
- **Options 1 and 2 keep SQLite + drizzle** — never leave the known stack. Option 1 keeps the
  _server_ model; Option 2 moves it _client-side_. That is the only real fork.
...
- **Performance inverts with the network.** Server/cloud options (0/1/3/4) pay a round-trip on
  ...
  state — that's the case _for_ the eventual rewrite, not against it.
