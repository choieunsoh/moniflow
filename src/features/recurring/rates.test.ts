...
// The catalog's valid codes for these tests — intentionally excludes XYZ, so the "unknown currency"
// case stays covered.
...
describe('resolveRate', () => {
  it('uses a pinned rate without touching the network', async () => {
    ...
    expect(await resolveRate(d, { currency: 'USD', rate: 36.5 }, '2026-07-05', CODES)).toBe(36.5);
  ...
  it('fetches the fixing for the DUE DATE, not today, and layers the card fee', async () => {
    ...
    const rate = await resolveRate(d, { currency: 'USD', rate: null }, '2026-07-05', CODES);
  ...
  it('falls back to the cached rate when the fetch fails — never blocks the ledger', async () => {
    ...
    expect(await resolveRate(d, { currency: 'USD', rate: null }, '2026-07-05', CODES)).toBeCloseTo(
      35 * 1.025,
      6,
    );
  });
  ...
  it('throws when there is neither a fetched nor a cached rate', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('offline'));
    await expect(
      resolveRate(d, { currency: 'USD', rate: null }, '2026-07-05', CODES),
    ).rejects.toThrow(/USD/);
  });
  ...
  it('rejects a currency the app does not know', async () => {
    const d = await db();
    await expect(
      resolveRate(d, { currency: 'XYZ', rate: null }, '2026-07-05', CODES),
    ).rejects.toThrow(/XYZ/);
  });
});
...
describe('convertAmount', () => {
  it('negates a plain THB rule and stores no originalAmount', async () => {
    const d = await db();
    expect(
      await convertAmount(d, { currency: 'THB', rate: null, amount: 2000 }, '2026-07-01', CODES),
  ...
  it('treats a null currency as THB (legacy rows)', async () => {
    const d = await db();
    expect(
      await convertAmount(d, { currency: null, rate: null, amount: 2000 }, '2026-07-01', CODES),
  ...
  it('converts a foreign rule and stores originalAmount SIGNED, matching entry-form', async () => {
    const d = await db();
    const got = await convertAmount(
      d,
      { currency: 'USD', rate: 36.5, amount: 9.99 },
      '2026-07-05',
      CODES,
    );
    expect(got).toEqual({
