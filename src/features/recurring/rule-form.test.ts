...
describe('parseRuleForm', () => {
  ...
  it("starts NEXT month when this month's day has already passed", () => {
    ...
    const got = parseRuleForm(fd(valid), '2026-07-20', CODES);
    expect(got).toMatchObject({ ok: true, rule: { startDate: '2026-08-05' } });
  });

  it('starts today when the day IS today', () => {
    expect(parseRuleForm(fd(valid), '2026-07-05', CODES)).toMatchObject({
      ok: true,
  ...
  it('pre-clamps startDate for a 31st rule starting in a short month', () => {
    expect(parseRuleForm(fd({ ...valid, day: '31' }), '2026-02-01', CODES)).toMatchObject({
      ok: true,
  ...
  it('parses an installment with a pinned rate', () => {
    expect(
      parseRuleForm(
        fd({ ...valid, totalCount: '12', startSeq: '4', rate: '36.5' }),
        '2026-07-01',
        CODES,
      ),
  ...
  it('rejects a blank name', () => {
    expect(parseRuleForm(fd({ ...valid, name: '  ' }), '2026-07-01', CODES)).toEqual({
      ok: false,
  ...
  it('rejects a day outside 1..31', () => {
    expect(parseRuleForm(fd({ ...valid, day: '32' }), '2026-07-01', CODES)).toEqual({
      ok: false,
      error: 'Day must be between 1 and 31.',
    });
    expect(parseRuleForm(fd({ ...valid, day: '0' }), '2026-07-01', CODES)).toEqual({
      ok: false,
      error: 'Day must be between 1 and 31.',
    });
  });
  ...
  it('rejects a non-positive amount — a rule is an expense', () => {
    expect(parseRuleForm(fd({ ...valid, amount: '0' }), '2026-07-01', CODES)).toEqual({
      ok: false,
      error: 'Amount must be greater than zero.',
    });
    expect(parseRuleForm(fd({ ...valid, amount: '-5' }), '2026-07-01', CODES)).toEqual({
      ok: false,
      error: 'Amount must be greater than zero.',
    });
  });
  ...
  it('rejects an unknown currency', () => {
    expect(parseRuleForm(fd({ ...valid, currency: 'XYZ' }), '2026-07-01', CODES)).toEqual({
      ok: false,
  ...
  it('rejects an interval other than monthly or yearly', () => {
    expect(parseRuleForm(fd({ ...valid, intervalMonths: '3' }), '2026-07-01', CODES)).toEqual({
      ok: false,
  ...
  it('rejects a startSeq past totalCount — nothing would ever post', () => {
    expect(
      parseRuleForm(fd({ ...valid, totalCount: '12', startSeq: '13' }), '2026-07-01', CODES),
    ).toEqual({ ok: false, error: 'The next payment number is past the total.' });
  });

  it('rejects a blank account or category', () => {
    expect(parseRuleForm(fd({ ...valid, account: '' }), '2026-07-01', CODES)).toEqual({
      ok: false,
      error: 'Choose an account.',
    });
    expect(parseRuleForm(fd({ ...valid, category: '' }), '2026-07-01', CODES)).toEqual({
      ok: false,
  ...
  describe('yearly rules carry a renewal month', () => {
    ...
    it('requires a month when the cadence is yearly', () => {
      expect(parseRuleForm(fd(yearly), '2026-07-01', CODES)).toEqual({
        ok: false,
    ...
    it('rejects a month outside 1..12', () => {
      expect(parseRuleForm(fd({ ...yearly, month: '13' }), '2026-07-01', CODES)).toEqual({
        ok: false,
        error: 'Choose which month a yearly rule renews in.',
      });
    });
    ...
    it('builds startDate from the chosen month + day, this year if it has not passed', () => {
      ...
      expect(
        parseRuleForm(fd({ ...yearly, month: '3', day: '5' }), '2026-01-10', CODES),
      ).toMatchObject({
        ok: true,
    ...
    it('rolls to next year when the chosen month + day has already passed', () => {
      ...
      expect(
        parseRuleForm(fd({ ...yearly, month: '3', day: '5' }), '2026-07-10', CODES),
      ).toMatchObject({
        ok: true,
    ...
    it('ignores an accidental month on a monthly rule', () => {
      ...
      expect(parseRuleForm(fd({ ...valid, month: '9' }), '2026-07-01', CODES)).toMatchObject({
        ok: true,
